pub mod print_data {
    ...
    pub fn print_data(day_config: &Vec<(NaiveTime, NaiveTime)>, current_time: NaiveTime, mode: Option<&str>, use_24_hour_format: bool, enable_ongoing: bool) {
        match mode {
            Some("raw") => print_raw(day_config, current_time, use_24_hour_format, enable_ongoing),
            Some("relative") => print_relative(day_config, current_time, enable_ongoing),
            Some("normal") => print_normal(day_config, current_time, use_24_hour_format, enable_ongoing),
            None => print_normal(day_config, current_time, use_24_hour_format, enable_ongoing),
            _ => println!("Invalid mode. Please use --help to see valid modes.")
        }
    }

    fn print_normal(day_config: &Vec<(NaiveTime, NaiveTime)>, current_time: NaiveTime, use_24_hour_format: bool, enable_ongoing: bool) {
        ...
        for (index, timeset) in day_config.iter().enumerate() {
            if compare_times(timeset.0, timeset.1, current_time) && enable_ongoing {
    ...
    fn print_raw(day_config: &Vec<(NaiveTime, NaiveTime)>, current_time: NaiveTime, use_24_hour_format: bool, enable_ongoing: bool) {
        if day_config == &vec![(NaiveTime::from_hms_opt(0, 0, 0).unwrap(), NaiveTime::from_hms_opt(0, 0, 0).unwrap())] {
            return;
        }
        let format = match use_24_hour_format {
    ...
    fn print_relative(day_config: &Vec<(NaiveTime, NaiveTime)>, current_time: NaiveTime, enable_ongoing: bool) {
        if day_config == &vec![(NaiveTime::from_hms_opt(0, 0, 0).unwrap(), NaiveTime::from_hms_opt(0, 0, 0).unwrap())] {
            println!("No schedule for today");
            return;
        }
